# Эксперимент glass: проверка оформления · 2026-09-26

Ветка `experiment/glass-workspace`, отдельный worktree `.tms-glass-frontend`. Область: общая оболочка, репозиторий, карточка кейса и дашборд. Production не изменён. Заключения о предыдущих задачах ниже не означают принятие этого эксперимента.
