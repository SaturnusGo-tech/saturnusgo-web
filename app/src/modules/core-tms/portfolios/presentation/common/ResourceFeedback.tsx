...
export function ResourceFeedback({ loading, error, copy, retry, hasContent = false }: { loading: boolean; hasContent?: boolean; error: TmsMutationFailure | null; copy: PortfolioCopy; retry: () => void }) {
  return <>
    {loading && (hasContent ? <span className={shell.srOnly} role="status">{copy.loading}</span> : <ContentSkeleton compact variant="list" label={copy.loading} />)}
