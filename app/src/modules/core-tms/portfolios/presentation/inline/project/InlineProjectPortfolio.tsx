...
export function InlineProjectPortfolio({ workspaceId, value, name, disabled, onChange, copy }: {
 ...
 return <div className={styles.inlineChoice}><AnimatedSelect label={copy.portfolio} value={value ?? ""} options={options.map(option => ({ ...option, icon: option.value ? <PortfolioIcon size={17} /> : undefined }))} disabled={disabled} onChange={value => onChange(value || null)} />
