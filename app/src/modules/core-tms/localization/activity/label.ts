...
const activityLabels: Record<string, readonly [string, string]> = {
  ...
  "run.complete": ["Test run completed", "Тест-ран завершён"],
  "run.start": ["Test run started", "Тест-ран запущен"],
  "run.pause": ["Test run paused", "Тест-ран приостановлен"],
  "run.resume": ["Test run resumed", "Тест-ран возобновлён"],
  "run.abort": ["Test run aborted", "Тест-ран прерван"],
  "run.verification_created": ["Verification run created", "Создан прогон для повторной проверки"],
  "run_item.retested": ["Case retest started", "Начата повторная проверка кейса"],
  "run_item.status_changed": ["Case result updated", "Изменён результат кейса в прогоне"],
  "run_step.result_changed": ["Step result recorded", "Записан результат шага"],
