...
import { isClosedDefect, matchesDefectScope } from "../model/defect-browser";
...
const counts = (items: Defect[]): DefectCounts => ({ total: items.length,
  open: items.filter((item) => !isClosedDefect(item.status)).length,
  critical: items.filter((item) => !isClosedDefect(item.status) && item.severity === "critical").length });
  ...
  return {
    async groups(query) {
      const items = defects.filter((item) => item.projectId === query.projectId &&
        matchesDefectScope(item.status, query.scope) && matches(item, query.q));
    ...
    async records(query, component) {
      const items = defects.filter((item) => item.projectId === query.projectId &&
        item.component === component && matchesDefectScope(item.status, query.scope) && matches(item, query.q));
