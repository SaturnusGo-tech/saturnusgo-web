...
import passwordStyles from "./profile-password.module.css";
...
export function ProfilePasswordForm({ profile, client, onSaved }: { readonly profile: Profile; readonly client: AdministrationPort; readonly onSaved: () => void }) {
  ...
  return <form className={passwordStyles.form} onSubmit={(event) => {
    ...
    <p className={passwordStyles.hint}>{locale === "ru" ? "Используйте не менее 12 символов для нового пароля." : "Use at least 12 characters for your new password."}</p>
    <div className={passwordStyles.fields}>
      ...
      {profile.mfaEnabled && <><AccessField label={copy.mfaCode} required autoComplete="one-time-code" disabled={command.pending}
        ...
        <label className={passwordStyles.recovery}><input type="checkbox" checked={recovery} onChange={(event) => setRecovery(event.target.checked)} />{copy.useRecovery}</label></>}
    </div>
    ...
    <div className={styles.actions}><button type="submit" className={styles.primary} disabled={command.pending}>{copy.updatePassword}</button></div>
  </form>;
}
