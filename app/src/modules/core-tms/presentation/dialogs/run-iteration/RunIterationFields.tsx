...
export function RunIterationFields({ state, workspaceId, offline, ru }: {
  ...
  return <section className={css.iteration}>
    <label><span>{ru ? "Итерация" : "Iteration"}</span><AnimatedSelect label={ru ? "Итерация" : "Iteration"}
    ...
    {!state.iterationId && <>
      <label><span>{ru ? "Название итерации" : "Iteration title"}</span><input required maxLength={200}
      ...
      <details className={css.details}><summary>{ru ? "Описание и теги" : "Description and tags"}</summary>
        ...
        <label><span>{ru ? "Теги" : "Tags"}</span><input value={state.tags} onChange={(event) => state.setTags(event.target.value)}
    ...
    <details className={css.details}><summary>{ru ? "Сборка и ответственный" : "Build and assignee"}</summary>
      <div className={css.settings}><label><span>{ru ? "Сборка" : "Build"}</span><input maxLength={500} value={state.build}
