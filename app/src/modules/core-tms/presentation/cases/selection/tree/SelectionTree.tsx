...
type Props = {
  ...
  ru: boolean; disabled?: boolean; selectable?: boolean; activeId?: string;
...
export function SelectionTree(props: Props) {
  const tree = useMemo(() => {
    const result = buildFolderTree(props.folders, props.cases);
  ...
  return <div className={`${css.repository} ${local.root}`} data-selection={props.selectable || undefined}
    ...
    <div className={css.treeScroll}><ul className={css.tree}>
      {tree.roots.map((node) => <RepositoryFolderBranch key={node.folder.id} node={node} depth={0}
        ...
        ru={props.ru} locked={Boolean(props.disabled)} canManage={false} canSelect={props.selectable}
      ...
      {tree.unfiled.map((item) => <RepositoryCaseLeaf key={item.id} item={item} depth={0}
        ...
        trailing={props.trailing?.(item)} accessory={props.accessory?.(item)} locked={Boolean(props.disabled)} canManage={false} canSelect={props.selectable} onOpen={open} onToggle={props.onToggle} />)}
    </ul></div>
  </div>;
}
