...
function harness(components = ["Payments", "Sign in"]) {
  ...
  const expansion = h.load(new URL("../../../defects/browser/state/expansion/useDefectExpansion.ts", import.meta.url));
  ...
  const { DefectBrowser: View } = h.load<{ DefectBrowser: typeof DefectBrowser }>(new URL("../browser/DefectBrowser.tsx", import.meta.url), name => {
    ...
    if (name.endsWith("useDefectExpansion")) return expansion;
...
test("an expanded group loads again after a new sorted result replaces the same group names", () => {
  const f = harness(); f.render(); f.render(); assert.deepEqual(f.opened, ["Payments", "Sign in"]);
  ...
  assert.equal(f.opened.filter(value => value === "Payments").length, 2);
  f.h.dispose();
});
...
test("several branches remain expanded independently and each row selects its real defect ID", () => {
  ...
  assert.equal(branches.filter(node => node.props.expanded).length, 2);
