  ...
  return (
    <div className={styles.twoPane}>
      ...
      <section className={`${styles.pane} ${styles.largePane}`}>
        {!selectedSuite ? <div className={styles.workbenchBlank}><strong>{t("suite.empty")}</strong><span>{t("suite.emptyHint")}</span><button className={styles.primaryButton} onClick={onCreate}><Plus size={16} /> {t("suite.create")}</button></div> : !detail ? <TessiqLoader pane label={t("common.loading")} testId="suite-detail-loading" /> : <>
          ...
          <div className={styles.suiteScopeLine}><span>{detail.type === "dynamic" ? `${localizedLabel(locale, "dynamic")}: ${(detail.filter.tags ?? []).join(", ")}` : t("suite.staticSelection")}</span><strong>{formatCount(locale, detail.resolvedCaseCount, ["case", "cases"], ["кейс", "кейса", "кейсов"])}</strong></div>
