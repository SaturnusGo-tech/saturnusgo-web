...
export function RunsView({ offline, runs, selectedRun, selectedItem, progress, onSelectRun, onSelectItem, onCreate, onStepStatus, onStepActual, onItemStatus, onComplete, onDefectCreated }: RunsViewProps) {
  ...
  return <div className={styles.executionShell} data-testid="runs-view">
        ...
        {executionEntries.map((step) => {
          ...
          return <article className={`${styles.executionStep} ${styles[`step_${status}`]}`} key={step.id}>
            ...
            <div className={styles.stepStatus}>
              ...
              <div className={styles.stepActions}><button disabled={selectedRun.status !== "active"} aria-label={`${t("runs.passStep")} ${step.order}`} title={t("runs.passStep")} className={status === "passed" ? styles.actionPassActive : ""} onClick={() => onStepStatus(step.id, "passed")}><Check size={15} /></button><button disabled={selectedRun.status !== "active"} aria-label={`${t("runs.failStep")} ${step.order}`} title={t("runs.failStep")} className={status === "failed" ? styles.actionFailActive : ""} onClick={() => onStepStatus(step.id, "failed")}><X size={15} /></button><button disabled={selectedRun.status !== "active"} aria-label={`${t("runs.blockStep")} ${step.order}`} title={t("runs.blockStep")} className={status === "blocked" ? styles.actionBlockActive : ""} onClick={() => onStepStatus(step.id, "blocked")}><Ban size={14} /></button></div>
            </div>
          </article>;
        })}
      </div>
    ...
    <footer className={styles.executionFooter}>
      <div className={styles.executionPager}><button className={styles.textButton} disabled={currentIndex <= 0} onClick={() => onSelectItem(selectedRun.items[currentIndex - 1]?.id)}><ChevronLeft size={16} /> {t("runs.previous")} <kbd>⌘ [</kbd></button><button className={styles.textButton} disabled={currentIndex >= selectedRun.items.length - 1} onClick={() => onSelectItem(selectedRun.items[currentIndex + 1]?.id)}>{t("runs.next")} <ChevronRight size={16} /><kbd>⌘ ]</kbd></button></div>
      <div className={styles.executionActions}><button className={styles.secondaryButton} onClick={() => onItemStatus("blocked")} disabled={selectedRun.status !== "active"}><Ban size={16} /> {t("runs.block")} <kbd>B</kbd></button><button className={styles.dangerButton} onClick={() => onItemStatus("failed")} data-testid="fail-case" disabled={selectedRun.status !== "active"}><XCircle size={16} /> {t("runs.fail")} <kbd>F</kbd></button><button className={styles.successButton} onClick={() => onItemStatus("passed")} data-testid="pass-case" disabled={selectedRun.status !== "active" || !canPass} title={!canPass ? t("runs.passRequiredFirst") : undefined}><CheckCircle2 size={16} /> {t("runs.pass")} <kbd>P</kbd></button><label className={styles.secondaryButton}><Paperclip size={16} /> {t("runs.addEvidence")} {evidence.length > 0 && <span>{evidence.length}</span>}<kbd>E</kbd><input id={`run-evidence-${selectedItem.id}`} type="file" multiple accept="image/*,video/*,.txt,.log,.pdf" onChange={(event) => void addEvidence(Array.from(event.target.files ?? []))} /></label>{failed && <button className={styles.reportButton} type="submit" form={`defect-form-${selectedItem.id}`} onClick={() => setReporting(true)} data-testid="report-defect"><Bug size={16} /> {reporting ? t("runs.createBug") : t("runs.reportBug")} <kbd>R</kbd></button>}</div>
      {evidenceError && <FormError message={evidenceError} />}
      <div className={styles.executionCompletion}>{selectedRun.status === "completed" ? <span className={`${styles.statusPill} ${styles.status_passed}`}><CheckCircle2 size={15} /> {t("runs.completed")}</span> : canComplete ? <button className={styles.primaryButton} onClick={onComplete}><CheckCircle2 size={16} /> {t("runs.complete")}</button> : <span>{t("runs.percentComplete", { percent: progress })}</span>}</div>
    </footer>
  </div>;
}
