...
import { PiCaretDown, PiCheck, PiMagnifyingGlass, PiUserCircle } from "react-icons/pi";
...
export function ResponsiblePicker({ workspaceId, value, onChange, disabled = false, offline = false, selectedName }: {
  ...
  return <div ref={root} className={styles.root} onKeyDown={(event) => {
    ...
    <button ref={trigger} className={styles.trigger} type="button" disabled={disabled || offline} aria-label={label}
      aria-haspopup="listbox" aria-expanded={open} aria-controls={listId} onClick={() => setOpen((current) => !current)}>
      <PiUserCircle size={22} aria-hidden="true" /><span>{value ? selected || (ru ? "Назначенный участник" : "Assigned member") : empty}</span><PiCaretDown aria-hidden="true" />
    </button>
    {open && <div className={styles.popover}>
      ...
      <div id={listId} role="listbox" aria-label={label} className={styles.options} onKeyDown={(event) => {
        ...
        <button type="button" role="option" aria-selected={!value} onClick={() => choose(null)}><span>{empty}</span>{!value && <PiCheck />}</button>
        {members.items.map((member) => <button type="button" key={member.id} role="option" aria-selected={value === member.id} onClick={() => choose(member.id, member.name)}>
          <span>{member.name}<small>{member.email}</small></span>{value === member.id && <PiCheck />}
        </button>)}
      </div>
