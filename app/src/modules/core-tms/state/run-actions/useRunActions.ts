...
import { createRunStepWriter } from "../../application/runs/step/saveRunStep";
export { stepMutationEvidence } from "../../application/runs/step/saveRunStep";
...
export function useRunActions(
  ...
  const stepWriter = useRef<ReturnType<typeof createRunStepWriter> | null>(null);
  if (!stepWriter.current) stepWriter.current = createRunStepWriter(http);
  const owner = JSON.stringify([state.data.workspace.id, state.projectId, derived.selectedRun?.id, derived.selectedRunItem?.id, state.connection]);
  ...
  async function setStepStatus(stepId: string, status: ExecutionStatus, actualResult?: string) {
    ...
    try {
      await itemMutations.run(item.id, async (current) => {
        ...
        if (currentOwner.current !== owner) throw new Error("Run selection changed.");
        try {
          const refreshed = await stepWriter.current!.write({ run, resource: current,
            stepId, status, actualResult, defaults: {
  ...
  function updateStepActualResult(stepId: string, value: string) {
    const item = derived.selectedRunItem;
    if (currentOwner.current !== owner || !item || !canEditRunAttempt(derived.selectedRun, item)) return;
