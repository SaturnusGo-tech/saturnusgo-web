...
function harness() {
  ...
  const module = { exports: {} as { WorkspaceStage: (props: { model: typeof model }) => { props: { onOpenRow: (row: Row) => void } } } };
...
test("dashboard item click persists exact run URL before state changes or passive effects", () => {
  const h = harness();
  h.click({ entity: "run_item", projectId: "p", id: "item-target", runId: "run-target", runItemId: "item-target" });
...
test("dashboard run click uses the run identity and clears prior item selection", () => {
  const h = harness();
  h.click({ entity: "run", projectId: "p", id: "run-only" });
...
test("workspace-wide run item navigates to its exact project without a stale project closure", () => {
  const h = harness();
  h.click({ entity: "run_item", projectId: "other-project", id: "other-item", runId: "other-run", runItemId: "other-item" });
  assert.deepEqual(h.events, ["assign"]);
...
test("cross-project defect navigation retains its existing scoped report link", () => {
  const h = harness();
  h.click({ entity: "defect", projectId: "other-project", id: "defect-target" });
  assert.deepEqual(h.events, ["assign"]);
