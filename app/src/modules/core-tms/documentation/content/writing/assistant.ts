...
export const writingAssistantArticle: DocArticle = {
  ...
  sections: [
    ...
    section("dictation", "Продиктуйте команду", paragraph("Нажмите микрофон рядом со стрелкой отправки и разрешите доступ к микрофону. Продиктованный текст добавится к уже введённой команде. Нажмите **Остановить диктовку**, проверьте формулировку и отправьте запрос. Диктовка сама не отправляет запрос и не заменяет текст в документе."),
      paragraph("Распознавание выполняет браузер, который может передавать аудио своей службе. Поддержка зависит от браузера, устройства и языка интерфейса Falcon. Если функция недоступна, используйте системную диктовку в поле команды или введите текст. При закрытии окна и уходе с вкладки микрофон отключается. Одна запись длится до минуты; затем можно продолжить новой записью.")),
