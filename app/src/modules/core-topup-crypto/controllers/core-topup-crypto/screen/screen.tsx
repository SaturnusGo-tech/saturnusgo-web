...
const ADDR = {
  ...
  USDT_ERC20:
    process.env.NEXT_PUBLIC_DEP_ADDR_USDT_ERC20 || "0x_your_usdt_erc20_addr",
  USDT_TRC20:
    process.env.NEXT_PUBLIC_DEP_ADDR_USDT_TRC20 || "T_your_usdt_trc20_addr",
  USDT_BEP20:
    process.env.NEXT_PUBLIC_DEP_ADDR_USDT_BEP20 || "0x_your_usdt_bep20_addr",
...
const fmtCrypto = (n: number, a: Asset) =>
  n.toFixed(DECIMALS[a]).replace(/\.?0+$/, "");
const sanitize16 = (s: string) => s.replace(/\D/g, "").slice(0, 16);
const group16 = (s: string) =>
  s
    .replace(/\D/g, "")
    .slice(0, 16)
    .replace(/(.{4})/g, "$1 ")
...
export default function CryptoTopUp() {
  ...
  const amountCrypto = useMemo(
    () => (price > 0 ? clamp(usd / price, 0, 1e16) : 0),
    [usd, price],
  );
  const address = useMemo(
    () => resolveDepositAddress(asset, net),
    [asset, net],
  );
  ...
  async function verifyAndLoadOptions() {
    ...
    try {
      ...
      const initCrypto =
        price > 0
          ? clamp(parseLocaleNumber(fiatStr) / prices[firstAsset], 0, 1e16)
          : 0;
  ...
  async function confirmTopup() {
    ...
    try {
      ...
      setTxId(
        data.txId ||
          (await sha256Hex(`${intentId}:${Date.now()}`)).slice(0, 16),
      );
  ...
  return (
    <main className="topup">
      <section className="topupHero" aria-labelledby="topup-title">
        <span className="topupKicker">SaturnusGo / Wallet</span>
        <h1 id="topup-title">Card top-up with a clear payment path.</h1>
        <p>
          The crypto top-up flow keeps the operational form, but the page now
          follows the same dark entry and light working area as the main screen.
        </p>
      </section>

      <section className="topupWorkArea">
        <div className="screen">
          {/* Header */}
          <div className="header">
            <div className="title">Card top-up</div>
            <div className="limit">
              Card load limit&nbsp; <b>$ 5 000</b>
            </div>
          </div>
            ...
            <section>
              ...
              <div className="kv">
                <div>
                  ...
                  <div className="kvSub">
                    We will pick the best rate at the moment we receive funds
                    from you
                  </div>
                </div>
                <div className="kvVal">
                  1 {asset} ≈ {neatUSD(price || 0)}
                </div>
              </div>
              ...
              <div className="kv">
                <div>
                  ...
                  <div className="kvSub">
                    All commission are included in the amount you pay
                  </div>
                </div>
                <div className="kvVal">
              ...
              <div className="kv">
                ...
                <div className="badgeSelect">
                  <select
                    value={net}
                    onChange={(e) => setNet(e.target.value as NetId)}
                  >
                    {availableNets.map((n) => (
            ...
            <section>
              <div className="kvTop">
                ...
                <div className="valBig">
                  {fmtCrypto(amountCrypto, asset)} {asset}
                </div>
              </div>
              ...
              {confirmOk && (
                <>
                  ...
                  <div className="okText">
                    Funds credited:{" "}
                    <b>
                      {neatUSD(confirmOk.amount)} {confirmOk.currency}
                    </b>
                  </div>
                  {txId && (
                    <div className="box">
      ...
      <style jsx>{`
        .topup {
          ...
          --topup-paper: #f2eee4;
          --topup-ink: #080808;
          --topup-muted: rgba(8, 8, 8, 0.62);
          min-height: 100dvh;
          margin-top: calc(var(--app-header-h, 96px) * -1);
          background: var(--topup-night);
          color: var(--topup-ink);
          overflow-x: clip;
        }
        ...
        .topupHero {
          ...
          display: grid;
          align-content: end;
          padding: calc(var(--app-header-h, 96px) + clamp(76px, 8vw, 118px))
            clamp(20px, 6vw, 78px) clamp(66px, 8vw, 104px);
          background:
            ...
            url("/mock/device-preview.jpg") center top / cover;
          color: #f7f4ee;
        }
        ...
        .topupWorkArea {
          ...
          padding: clamp(76px, 9vw, 126px) 20px clamp(90px, 11vw, 150px);
          background: var(--topup-paper);
        }
        .screen {
          width: min(440px, 100%);
          max-width: 440px;
        ...
        .success {
          ...
          gap: 8px;
          align-items: center;
          background: rgba(11, 107, 66, 0.1);
          color: #0b6b42;
          border: 1px solid rgba(11, 107, 66, 0.22);
          border-radius: 10px;
          padding: 8px 10px;
          font-weight: 700;
          margin: 6px 2px 8px;
